...
func _ai_turn() -> bool:
	...
	# patrol
		...
		elif !map.can_approach(position, map.map_to_local(patrol_route[patrol_pointer]), 9999) and map.can_see(position, patrol_route[patrol_pointer], int(int(aggro_range) / 2)):
			patrol_pointer = (patrol_pointer + 1) % patrol_route.size()
		else:
			if select_type != SELECT_TYPE_WALK:
				select(SELECT_TYPE_WALK)
